(()=>{
  ...
  .partner .controls,.partner .claimbox,.partner .b-detail,.partner .b-stock{display:none!important}.partner.b-open .controls{display:grid!important}.partner.b-open .claimbox,.partner.b-open .b-detail,.partner.b-open .b-stock{display:block!important}
  ...
  const partnerCodeOf=card=>{
    const t=(card.querySelector('.partnerhead')?.textContent||'').trim();
    const m=t.match(/\b([A-Z0-9_-]{2,})\b/i);
  ...
  async function loadAllocations(){
    ...
    const{data,error}=await stockDb.rpc('admin_active_voucher_allocations');
  ...
  async function renderStock(card){
    ...
    try{
      const code=partnerCodeOf(card);
      const rows=(await loadAllocations()).filter(x=>String(x.partner_code||'').toUpperCase()===code);
      if(!rows.length){
        box.innerHTML='<div class="b-stock-title">Allocated Voucher Stock</div><div class="b-stock-sub">Revoke only unissued stock.</div><div class="b-empty">No active allocations.</div>';
        return;
      }
      box.innerHTML='<div class="b-stock-title">Allocated Voucher Stock</div><div class="b-stock-sub">Revoke only unissued stock. Issued customer vouchers are not changed.</div>' + rows.map(r=>{
        const rem=Number(r.remaining_unissued||0);
        return '<div class="b-allocation" data-allocation="'+esc(r.allocation_id)+'">'
          +'<div class="b-allocation-grid">'
          +'<div class="b-cell"><span>Voucher Version</span><b>'+esc(r.version_name||r.version_id)+'</b></div>'
          +'<div class="b-cell"><span>Allocated</span><b>'+Number(r.quantity_allocated||0)+'</b></div>'
          +'<div class="b-cell"><span>Issued</span><b>'+Number(r.issued_count||0)+'</b></div>'
          +'<div class="b-cell"><span>Unissued</span><b>'+rem+'</b></div></div>'
          +(rem>0?'<div class="b-revoke-row"><input class="b-qty" type="number" min="1" max="'+rem+'" placeholder="Qty"><input class="b-reason" placeholder="Reason"><button type="button" class="b-revoke">Revoke</button></div>':'<div class="b-empty">No unissued stock to revoke.</div>')
          +'</div>';
      }).join('');
      box.querySelectorAll('.b-revoke').forEach(btn=>btn.onclick=async()=>{
        const row=btn.closest('.b-allocation');
        const qtyInput=row.querySelector('.b-qty');
        const qty=Number(qtyInput.value), max=Number(qtyInput.max);
        const reason=row.querySelector('.b-reason').value.trim();
        if(!Number.isInteger(qty)||qty<1||qty>max){alert('Enter a quantity from 1 to '+max+'.');return;}
        if(!reason){alert('Revoke reason is required.');return;}
        if(!confirm('Revoke '+qty+' unissued voucher(s)?\n\nIssued customer vouchers will remain unchanged.'))return;
        btn.disabled=true;btn.textContent='Revoking…';
        try{
          const{error}=await stockDb.rpc('admin_engine_revoke_unissued',{p_allocation_id:row.dataset.allocation,p_quantity:qty,p_reason:reason,p_actor_user_id:null});
          if(error)throw error;
          allocationRows=null;
          await renderStock(card);
          alert('Unissued voucher stock revoked.');
        }catch(e){alert(String(e?.message||e));btn.disabled=false;btn.textContent='Revoke';}
      });
  ...
  const enhance=()=>{
    ...
    root.querySelectorAll('.partner').forEach(card=>{
      ...
      btn.type='button';btn.className='b-manage';btn.textContent='Manage';
      ...
      btn.addEventListener('click',async()=>{
        ...
        root.querySelectorAll('.partner.b-open').forEach(other=>{other.classList.remove('b-open');const ob=other.querySelector('.b-manage');if(ob)ob.textContent='Manage';});
        if(opening){card.classList.add('b-open');btn.textContent='Close';await renderStock(card);card.scrollIntoView({behavior:'smooth',block:'start'});}
