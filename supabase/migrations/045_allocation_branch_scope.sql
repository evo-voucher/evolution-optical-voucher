-- Allocation-level branch scope
-- Root invariant: an Admin assignment may further restrict where its Vouchers can be redeemed.
-- Issued scope = Partner claim scope ∩ Version scope ∩ Allocation scope.
-- Existing allocations remain unrestricted for backward compatibility.
...
-- This trigger is deliberately on the owning branch-row layer. The canonical issue RPC
-- may materialise Partner/Version candidates; rows outside the Allocation scope are skipped,
-- leaving the final stored snapshot as the three-way intersection without reopening issued rows.
...
-- Publish wrapper that also persists Version branch scope atomically.
create or replace function public.admin_publish_voucher_version_v3(
  p_template_id uuid,
...
begin
  ...
  v_id:=public.admin_publish_voucher_version_v2(
    p_template_id,p_version_name,p_face_value,p_discount_percent,p_validity_mode,
    p_valid_days,p_valid_months,p_valid_until,p_min_spend,p_max_discount,p_usage_limit,
    p_transferable,p_terms_text,p_supply_limit,coalesce(p_all_branches,true),p_theme_code,
    coalesce(p_theme_config,'{}'::jsonb),p_greeting_text
  );
...
revoke all on function public.admin_publish_voucher_version_v3(uuid,text,numeric,numeric,text,integer,integer,date,numeric,numeric,integer,boolean,text,integer,boolean,text[],text,jsonb,text) from public,anon;
grant execute on function public.admin_publish_voucher_version_v3(uuid,text,numeric,numeric,text,integer,integer,date,numeric,numeric,integer,boolean,text,integer,boolean,text[],text,jsonb,text) to authenticated;

-- Scoped allocations are intentionally distinct lots. This avoids merging two Admin assignments
-- that carry different branch restrictions and keeps allocation-relative clocks deterministic.
create or replace function public.admin_engine_allocate_v3(
  p_partner_id uuid,
  p_version_id uuid,
  p_quantity integer,
...
begin
  ...
  values(v_actor,v_actor_name,'voucher_allocation_changed','voucher_allocation',v_allocation.id::text,p_partner_id,
    jsonb_build_object('version_id',p_version_id,'quantity_added',p_quantity,'quantity_allocated',p_quantity,
      'validity_anchor',v_anchor,'allocation_valid_days',case when v_anchor='allocation' then p_allocation_valid_days end,
  ...
  return jsonb_build_object('success',true,'allocation_id',v_allocation.id,'partner_id',p_partner_id,
    'version_id',p_version_id,'quantity_added',p_quantity,'quantity_allocated',p_quantity,'quantity_revoked',0,
    'validity_anchor',v_anchor,'allocation_valid_days',case when v_anchor='allocation' then p_allocation_valid_days end,
    'all_branches',coalesce(p_all_branches,true),'branch_codes',coalesce(p_branch_codes,array[]::text[]),
    'valid_from',v_valid_from,'valid_until',v_valid_until,'created',true);
end;
...
revoke all on function public.admin_engine_allocate_v3(uuid,uuid,integer,text,integer,boolean,text[],uuid) from public,anon;
grant execute on function public.admin_engine_allocate_v3(uuid,uuid,integer,text,integer,boolean,text[],uuid) to authenticated,service_role;
