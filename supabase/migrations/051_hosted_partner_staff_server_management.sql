-- Hosted Partner Staff server-management compatibility.
-- Aligns GitHub manage-partner-staff Edge Function with the existing hosted
-- partner_users/status model while keeping all mutation RPCs service_role-only.
...
begin
  ...
  insert into public.admin_audit_log(actor_user_id,action_type,entity_type,entity_id,partner_id,after_data,metadata)
  values(p_actor_user_id,'partner_staff_created','partner_staff',v_staff.id::text,v_partner.id,
...
begin
  ...
  where pu.user_id=p_actor_user_id and pu.role='partner_admin' and pu.status='active' and pu.removed_at is null limit 1;
  if not found then raise exception 'Active Partner Admin actor required'; end if;
  ...
  where pu.id=p_staff_id and pu.partner_id=v_partner.id and pu.role='partner_staff' for update;
  ...
  if v_action='rename' then
    ...
    if v_target.removed_at is not null or v_target.status='inactive' then raise exception 'Removed Staff cannot be reactivated'; end if;
    ...
    update public.partner_users set status='inactive',removed_at=v_now,updated_at=v_now where id=v_target.id returning * into v_target;
  ...
  insert into public.admin_audit_log(actor_user_id,action_type,entity_type,entity_id,partner_id,after_data,metadata)
  values(p_actor_user_id,v_action_type,'partner_staff',v_target.id::text,v_partner.id,
...
begin
  ...
  where pu.id=p_staff_id and pu.partner_id=v_actor.partner_id and pu.role='partner_staff'
    and pu.status<>'inactive' and pu.removed_at is null;
  if not found then raise exception 'Active Partner Staff target required'; end if;

  insert into public.admin_audit_log(actor_user_id,action_type,entity_type,entity_id,partner_id,after_data,metadata)
  values(p_actor_user_id,'partner_staff_password_reset','partner_staff',v_target.id::text,v_actor.partner_id,
