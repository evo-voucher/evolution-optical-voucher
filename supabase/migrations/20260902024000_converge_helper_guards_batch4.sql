-- Converge selected helper/trigger function definitions on voucher-stage to current Production.
-- Exact signatures only. Production remains read-only.
-- Sandbox-dependent guards are intentionally excluded because voucher-stage does not yet
-- contain public.admin_test_sandbox.
...
declare
  v_issued integer;
  v_hard_reduce boolean := coalesce(current_setting('evo.hard_reduce_allowed',true),'off')='on';
begin
  ...
  if tg_op='UPDATE' then
    ...
    if new.quantity_allocated < old.quantity_allocated and not v_hard_reduce then
  ...
  if new.quantity_allocated < v_issued + new.quantity_revoked then
    raise exception 'Allocated quantity cannot be lower than issued plus revoked quantity';
  end if;
...
REVOKE ALL ON FUNCTION public.guard_allocation_integrity() FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.guard_allocation_integrity() FROM authenticated;
REVOKE EXECUTE ON FUNCTION public.guard_allocation_integrity() FROM service_role;
