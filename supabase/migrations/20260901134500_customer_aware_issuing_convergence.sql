-- Converge customer-aware issuing and customer attachment with current production baseline.
...
create or replace function public.issue_engine_voucher_with_customer(
  p_version_id uuid,
  p_customer_name text,
  p_customer_phone text default null::text,
  p_partner_id uuid default null::uuid,
  p_customer_birthday date default null::date,
  p_customer_district text default null::text
)
returns jsonb
language plpgsql
security definer
...
-- Retire legacy issuing path and obsolete global partner quota trigger.
...
-- Align execute ACLs exactly with Production.
