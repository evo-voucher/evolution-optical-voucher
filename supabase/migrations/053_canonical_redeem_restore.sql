-- Canonical redeem restore after hosted compatibility.
-- Hosted production and canonical rebuilds use different redemption row contracts:
--   hosted: staff_user_id -> staff_users.id, redeem_method qr_scan/manual, status success/reversed
--   canonical: staff_user_id -> auth.users.id, redeem_method qr/manual_code/admin, status completed/reversed
-- 050 adapts hosted production. Restore the canonical 044 implementation only
-- when the canonical Voucher markers are present. Hosted production remains unchanged.
    ...
    execute $sql$
      ...
      begin
        ...
        if v_staff.role='all_branch_manager' then
          ...
          select b.id,b.branch_name into v_branch_id,v_branch_name
          from public.branches b
          where b.id=v_staff.branch_id and b.status='active'
          limit 1;
        end if;
        ...
        if v_voucher.status='expired' or v_voucher.expiry_date<v_today then
          ...
          set status=case when status='active' then 'expired' else status end,updated_at=v_now
        ...
        insert into public.redemptions(
          voucher_id,partner_id,branch_id,staff_user_id,staff_name_snapshot,redeem_method,status,redeemed_at,notes
        ) values (
          v_voucher.id,v_voucher.partner_id,v_branch_id,v_uid,v_staff.staff_name,v_method,'completed',v_now,
        ...
        insert into public.admin_audit_log(
          ...
          v_uid,v_staff.staff_name,'voucher_redeemed','redemption',v_redemption_id::text,v_voucher.partner_id,
          jsonb_build_object('voucher_id',v_voucher.id,'voucher_code',v_voucher.voucher_code,
            'branch_id',v_branch_id,'branch_name',v_branch_name,'usage_count',v_new_usage,'usage_limit',v_voucher.usage_limit),
          jsonb_build_object('redeem_method',v_method,
            'branch_scope_source',case when v_voucher.branch_scope_snapshotted then 'voucher_snapshot' else 'legacy' end)
        );

        return jsonb_build_object(
          'success',true,'redemption_id',v_redemption_id,'voucher_id',v_voucher.id,
          'voucher_code',v_voucher.voucher_code,'customer_name',v_voucher.customer_name,
          'voucher_type',v_voucher.voucher_type,'branch_id',v_branch_id,'branch_name',v_branch_name,
          'staff_name',v_staff.staff_name,'redeemed_at',v_now,'usage_count',v_new_usage,
          'usage_limit',v_voucher.usage_limit,'remaining_uses',greatest(0,v_voucher.usage_limit-v_new_usage),
