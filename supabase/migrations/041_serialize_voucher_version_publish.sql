-- Serialize version publication per voucher template.
-- Prevents concurrent Admin publishes from selecting the same next version_no.
...
begin
  ...
  -- Canonical concurrency boundary for publishing: lock the template row first.
  -- Every publication for the same template is therefore serialized before max(version_no)+1.
  select vt.status
