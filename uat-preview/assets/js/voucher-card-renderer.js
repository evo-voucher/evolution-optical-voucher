...
(function(){
  ...
  const gradient=(ctx,x,y,w,h)=>{const g=ctx.createLinearGradient(x,y,x+w,y+h);g.addColorStop(0,'#2547bd');g.addColorStop(.55,'#543ac7');g.addColorStop(1,'#8a43dc');return g};
  ...
  const drawPin=(ctx,x,y)=>{ctx.save();ctx.fillStyle=C.purple;ctx.beginPath();ctx.arc(x,y-5,10,0,Math.PI*2);ctx.fill();ctx.beginPath();ctx.moveTo(x-6,y);ctx.lineTo(x,y+13);ctx.lineTo(x+6,y);ctx.closePath();ctx.fill();ctx.fillStyle=C.white;ctx.beginPath();ctx.arc(x,y-5,3,0,Math.PI*2);ctx.fill();ctx.restore()};
  const drawGift=(ctx,x,y,w,h)=>{
    roundRect(ctx,x,y,w,h,26,C.softPurple,'#ddd5fb',2);
    ctx.save();ctx.translate(x+70,y+55);ctx.strokeStyle=C.blue;ctx.lineWidth=7;ctx.beginPath();ctx.ellipse(50,75,44,20,0,0,Math.PI*2);ctx.ellipse(145,75,44,20,0,0,Math.PI*2);ctx.moveTo(94,75);ctx.lineTo(101,75);ctx.stroke();ctx.restore();
    roundRect(ctx,x+42,y+128,150,108,18,'#d8e5ff','#a9bef5',2);roundRect(ctx,x+98,y+108,38,145,8,'#8ba4ef');
    text(ctx,'A little gift',x+220,y+120,35,600,C.purple,'left','Georgia');text(ctx,'just for you',x+220,y+167,35,600,C.blue,'left','Georgia');text(ctx,'♡',x+286,y+214,38,500,C.purple);
  };
  ...
  async function render(data){
    ...
    ctx.fillStyle='#eef4ff';ctx.fillRect(0,0,W,H);roundRect(ctx,24,24,W-48,H-48,44,C.white,'#d7e3f6',2);
    text(ctx,'Evolution Optical',W/2,105,48,800,C.navy,'center');text(ctx,'Sdn Bhd',W/2,145,27,600,C.purple,'center');
    roundRect(ctx,70,190,W-140,210,34,gradient(ctx,70,190,W-140,210));text(ctx,'FREE',105,268,66,900,C.white);text(ctx,'GLASSES',105,330,58,900,C.white);text(ctx,'VOUCHER',105,385,52,900,C.white);ctx.strokeStyle='rgba(255,255,255,.55)';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(605,230);ctx.lineTo(605,365);ctx.stroke();text(ctx,'VALUE',650,270,24,700,C.white);text(ctx,data.value_label||'RM60',650,350,76,900,C.white);
    ...
    drawGift(ctx,70,735,500,270);
    roundRect(ctx,600,725,410,305,26,C.white,'#7663d7',3);const qr=await qrCanvas(data.qr_value,250);ctx.drawImage(qr,680,750,250,250);roundRect(ctx,625,1003,360,54,18,gradient(ctx,625,1003,360,54));text(ctx,'SCAN TO REDEEM',805,1039,21,800,C.white,'center');
    text(ctx,'⌖  REDEEM AT',70,1108,25,800,C.purple);ctx.strokeStyle=C.line;ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(255,1100);ctx.lineTo(1005,1100);ctx.stroke();
    ...
    for(const cols of rows){const gap=24,areaW=W-140,colW=(areaW-gap*(cols-1))/cols;for(let c=0;c<cols&&idx<branches.length;c++,idx++){const b=branches[idx],x=70+c*(colW+gap);if(cols===1&&rows.length>1){drawPin(ctx,x+8,y+18);text(ctx,b.branch_name||b.branch_code||'Branch',x+28,y+23,23,800,C.ink);text(ctx,b.address||'',x+28,y+55,18,500,C.muted);text(ctx,b.phone?`Tel: ${String(b.phone).replace(/^Tel:\s*/i,'')}`:'',x+28,y+82,19,700,C.purple);continue}drawPin(ctx,x+8,y+18);text(ctx,b.branch_name||b.branch_code||'Branch',x+28,y+23,fitText(ctx,b.branch_name||b.branch_code||'Branch',colW-38,22,15,800),800,C.ink);ctx.font='500 17px Arial';const lines=wrap(ctx,b.address||'',colW-28).slice(0,3);lines.forEach((ln,i)=>text(ctx,ln,x+8,y+52+i*22,17,500,C.muted));text(ctx,b.phone?`Tel: ${String(b.phone).replace(/^Tel:\s*/i,'')}`:'',x+8,y+52+Math.min(lines.length,3)*22+7,17,700,C.purple)}y+=rowH}
    roundRect(ctx,70,1515,W-140,64,24,gradient(ctx,70,1515,W-140,64));text(ctx,'One-time redemption only  •  Non-exchangeable for cash  •  T&C apply',W/2,1557,19,700,C.white,'center');
    return canvas;
  }
