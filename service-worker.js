const SAFE_CACHE='evolution-voucher-safe-shell-v2';
const SAFE_SHELL=[
  './',
  './index.html',
  './admin.html',
...
self.addEventListener('fetch',event=>{const req=event.request;if(req.method!=='GET')return;const url=new URL(req.url);if(url.origin===self.location.origin){event.respondWith((async()=>{try{const res=await fetch(req,{cache:'no-store'});const cache=await caches.open(SAFE_CACHE);cache.put(req,res.clone());return res}catch(_){return (await caches.match(req))||(await caches.match('./index.html'))}})())}});